$(document).ready(function(){
    ...
    function ajaxcart(id, cantidad)
    {
        ...
        $.ajax({
            type: "POST",
            url: basePath + "sales/remove",
            data: {
                id: id
            },
            dataType: "json",
            success: function(data){
                $('#msg').html('<div class="alert alert-success flash-msg">Item Eliminado.</div>');
                ...
                if(data.pedidos == "")
                {
                    window.location.replace(basePath + "products/index");
                }
            },
            error: function(){
